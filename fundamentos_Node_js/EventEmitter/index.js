...
const readFile = async (path) => {
    ...
    try{
        ...
        bus.publish(Metric({name: 'readFileDuration', value:duration, unit: 'miliseconds'}))
    }  
}
...
const writeFile = async (path, content) => {
    ...
    try{
        ...
        const duration = Date.now() - startTime
        bus.publish(Metric({name: 'writeFileDuration', value:duration, unit: 'miliseconds'}))
    }  
}
...
const main = async () =>{
    ...
    await copy(
        ...
        join(__dirname, 'package.metrics.json')
    )
    ...
    await copy(
        ...
        join(__dirname, 'ping-pong-copy.js')
    )

    console.log('Procesoo finalizado')
}
